...
def generate_power_profile(duration, power_rated=120, curve_type=0):
    ...
    if curve_type == 0:
        ...
        if duration > decay_start:
            power_profile[decay_start:] = np.linspace(
                power_rated, power_rated * 0.5, duration - decay_start
            )
...
def generate_charging_requirements(
    num_ev, power_support, type_curve, battery_capacity=None
):
    """
...
def generate_ev_demand(num_evs, base_path, seed=None):
    ...
    # 加载分布数据
    arrival_values, arrival_dist = load_distribution_from_csv(
        os.path.join(base_path, "arrival_distribution.csv")
    )
    duration_values, duration_dist = load_distribution_from_csv(
        os.path.join(base_path, "duration_distribution.csv")
    )
    start_soc_values, start_soc_dist = load_distribution_from_csv(
        os.path.join(base_path, "start_soc_distribution.csv")
    )
    end_soc_values, end_soc_dist = load_distribution_from_csv(
        os.path.join(base_path, "end_soc_distribution.csv")
    )
    ...
    for i in range(num_evs):  # 按单辆生成
        # 生成到达时间
        arrive_time = min(
            int(np.random.choice(arrival_values, p=arrival_dist)), 96 - 1
        )  # 避免最后一个时间段的来车
        ...
        # 功率越大，电池容量往往越大
        min_capacity = (
            40 + ((power_support - 60) / 20) * 10
        )  # 功率每增加20kW，最小容量增加10kWh
        max_capacity = (
            70 + ((power_support - 60) / 20) * 15
        )  # 功率每增加20kW，最大容量增加15kWh
        battery_capacity = int(np.random.uniform(min_capacity, max_capacity))
        # battery_capacity = int(round(battery_capacity / 10) * 10)  # 改为整十

        # 生成起始和目标SOC
        start_soc = (
            np.random.choice(start_soc_values, p=start_soc_dist) / 100
        )  # 转换为0-1范围
        end_soc = (
            np.random.choice(end_soc_values, p=end_soc_dist) / 100
        )  # 转换为0-1范围
        ...
        # 计算所需的充电时间（以15分钟时段计）
        power_coefficient = (
            1.0 if curve_type == 0 else (0.85 if curve_type == 1 else 0.9)
        )
...
def generate_ev_parameters(Num_TimeSteps, Num_EVs, prob):
    ...
    arrival = np.random.choice(
        range(Num_TimeSteps - 16), size=Num_EVs, p=prob
    )  # 按照概率分布生成车辆到达时间
    stay_hours = np.random.choice(
        [1, 2, 3], Num_EVs, p=[0.7, 0.2, 0.1]
    )  # 随机的停留时间分布
    departure = {
        i: min(arrival[i] + stay_hours[i] * 4, Num_TimeSteps) for i in range(Num_EVs)
    }
    # 使用离散值生成充电功率 (60, 80, 100, 120 kW)
    power_levels = [60, 80, 100, 120]
    power_support = {i: np.random.choice(power_levels) for i in range(Num_EVs)}
    curve_type = {
        i: np.random.randint(0, 3) for i in range(Num_EVs)
    }  # 随机分配充电曲线类型
    # 将充电能量时间需求转换为充电时间需求
    required_charging_time, battery_capacity = generate_charging_requirements(
        Num_EVs, power_support, curve_type
    )
    return (
        arrival,
        departure,
...
if __name__ == "__main__":
    ...
    if not args.base_path:
        raise ValueError(
            "缺少 --base_path。请传入分布文件目录，或设置 DSSGYM_EV_DISTRIBUTION_PATH。"
        )
    ...
    if "-" in base_name:
        ...
        dist_location, dist_day_type = parts[0], parts[1]
    else:
    ...
    charge_duration_required = {
        i: ev_demand[i]["time_slots_needed"] for i in range(Num_EVs)
    }
    ...
    print(
        f"平均充电时长: {np.mean(list(charge_duration_required.values())) / 4:.2f} 小时"
    )
