...
> 开发者回应：浅拷贝引用的问题确实不是我刻意实现的，同时也是我没意识到的，之前有些运行结果不符合我的预期，可能也和这个有关。
...
| M11 | `reward_curve.py:78/102` | `plt.show()` 在无头环境下会阻塞或报错 |
...
| M20 | `convert_freq.py:81` | 帮助信息写的是 `convert_linecodes.py`，实际文件名是 `convert_freq.py` |
...
| **可维护性** | 5/10 | 模块级副作用（H3）、浅拷贝引用共享（H6）、主/legacy 路径分叉（H5/H10） |
